...
A separate `TestRunner.java` class was implemented to verify the correctness of all four data structures independently of the main demo (`Main.java`).

The following operations were tested:
- BST: search for an existing patient, search for a non-existing patient, delete a patient
- Queue: FIFO order on dequeue, dequeue on an empty queue
- Stack: LIFO order on pop, stack empty check after popping all records
- Linked List: search for an existing visit, search for a missing visit, remove a visit correctly

**Result: 12/12 tests passed.**

To run the tests: right-click `TestRunner.java` in Eclipse and select `Run As > Java Application`.
